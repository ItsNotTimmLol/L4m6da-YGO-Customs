...
--At least one of the two effects must be possible
function s.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local dim=Duel.IsExistingMatchingCard(
		function(tc)
...
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Change monsters you own to Defense Position
		...
		local dim=Duel.GetMatchingGroup(
			function(tc)
				return tc:IsCode(88438982)
					and tc:IsControler(tp)
					and tc:IsLocation(LOCATION_MZONE)
		...
		local g=Duel.GetMatchingGroup(
			s.negotargetfilter,tp,LOCATION_MZONE,0,nil,tp
		)
...
function s.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(
			s.ctfilter,tp,LOCATION_ONFIELD,0,1,nil
		)
	end
...
function s.ctop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(
		tp,s.ctfilter,tp,LOCATION_ONFIELD,0,1,1,nil
	)
	local tc=g:GetFirst()
