...
function s.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		return Duel.IsExistingTarget(s.eqfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
	end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc or not tc:IsFaceup() or not c:IsRelateToEffect(e) then
		return
	end
	-- The controller of the targeted monster performs the equip
	local p=tc:GetControler()
	if Duel.Equip(p,c,tc) then
		-- This card can only be equipped to the targeted monster
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetCode(EFFECT_EQUIP_LIMIT)
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetValue(function(e,card) return card==tc end)
		e1:SetReset(RESET_EVENT+RESETS_STANDARD)
		c:RegisterEffect(e1)
	end
end
